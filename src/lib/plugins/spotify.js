...
import S from 'spotify-web-api-node'
    ...
    return (
      <SvgIcon>
        <g fill="none" fillRule="evenodd">
          <g fill="none" fillRule="evenodd">
            <g className="spotify-icon" transform="translate(1.000000, 1.000000)">
              <g>
                <path
...
export default class Spotify extends Component {
  ...
  constructor(props) {
    super(props)
    this.state = { current: 'Not playing' }
  }
  ...
  handleClick = async e => {
    try {
      if (e.button == 0) {
        ...
        if (this.state.current === 'Not playing') {
          await spotify.play()
        } else {
          await spotify.pause()
        }
